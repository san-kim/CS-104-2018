...
//constructor
CPUL::CPUL(string const & playername, unsigned int maxTiles, TrieSet* wordsTrie, Dictionary* dictionary, Board* board) 
...
void CPUL::updateHandSize()
{
	set<Tile*> playerhand_ =  this->getHandTiles();
	this->maxTiles = playerhand_.size();
}

void CPUL::setAsFirstPlayOfGame()
{
	isFirstTurn = true;
}

void CPUL::undoFirstPlaySet()
{
	isFirstTurn = false;
}
...
vector<StartingSquare> CPUL::startingSquares()
{
...
void CPUL::backtrack_squares_horizontal(TrieNode* currentNode, vector<Tile*> currentWord, unsigned int lettersPlaced, unsigned int currentRow, 
...
//given a starting square, backtrack
void CPUL::backtrack_squares_vertical(TrieNode* currentNode, vector<Tile*> currentWord, unsigned int lettersPlaced, unsigned int currentRow, 
unsigned int currentCol, unsigned int targetlength, vector<Tile*>& tilesinHand, vector<PlaceMove*>& wordsFormed, StartingSquare& start)
{
	//for vertical
	//base case, either success or failure, should only backtrack up till this amount
...
PlaceMove* CPUL::chooseMove()
{
	...
	vector<PlaceMove*>::iterator longestMove = result_PlaceMoves.end();
	...
	for(it = result_PlaceMoves.begin(); it != result_PlaceMoves.end(); ++it)
	{
		...
		if(words_formed.size() > 0)
			wordLength = words_formed[words_formed.size()-1].first.size();
		...
		//if it is a legal word, compare length, if it is greater length, then update what the max PlaceMove is
	...
	if(maxLength == 0 || longestMove == result_PlaceMoves.end())
	{
		return NULL;
	}
	...
	{	//if it's not the longestMove's placeMove pointer then delete it
		if(result_PlaceMoves[i] != *longestMove)
		{

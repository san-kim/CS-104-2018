...
public:
	...
	Board (std::string board_file_namey, std::string init_file);
	...
	std::vector< std::pair<std::string, unsigned int> > getPlaceMoveResults(PlaceMove &m) const;
	...
	*/
	void executePlaceMove (PlaceMove & m);
	...
	bool isOccupiedSquare(size_t y, size_t x);
...
private:
	...
	int rows;
	int columns;
	std::vector< std::vector<Square*> > squareboard;
};

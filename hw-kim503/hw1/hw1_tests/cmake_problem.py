...
""" % test_name)
			...
			if match_list is None or len(match_list) < 1:
				...
				logging_tools.log_warning("cmake_problem.py: could not find valgrind results for test " + test_name, problem.logging_level)
			...
			for measurement_element in test_results_element['Results']['NamedMeasurement']:
				...
				if not passed and measurement_element['@name'] == 'Exit Value':
					...
					if int(measurement_element['Value']) == 127:
						# this exit code appears to indicate that the test was not run
						didnt_run = True
	...
	print("Writing formatted file: %s " % problem.formatted_file)

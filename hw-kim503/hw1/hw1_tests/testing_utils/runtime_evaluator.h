...
private:
	...
	std::chrono::microseconds accumulatedDuration;

	// time (in microseconds on CLOCK_MONOTONIC) that the clock was enabled (only valid if isEnabled is true)
	uint64_t currStartTime;

	bool isEnabled;

	// get the time on CLOCK_MONOTONIC in microseconds
...
public:
	...
	typedef std::function<std::chrono::microseconds(uint64_t, RandomSeed)> Snippet;
